...
typedef struct artist{
    ...
    int strokes; // The number of strokes an artist can make
}artist_t;
...
void* paint(void* args){
	...
	for(int i =0; i < painter->strokes; ++i){
        ...
        if (
            ...
            printf("Thread %u did NOT get the lock\n", pthread_self());
            painter->x = currentX;
            painter->y = currentY;
        }
    }
