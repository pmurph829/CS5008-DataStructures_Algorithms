...
//Traverse a tree using depth first search and perform some function
//at each node.
int dfs_traverse(bstnode_t* root, int (*func)(bstnode_t*), int acc) {
    
    // call the function on our current node.
    acc += func(root);
    
    // if both children are empty, return. Otherwise recurse.
    if (root->leftChild == NULL && root-> rightChild == NULL) {
        return acc;
    } 
    if (root->leftChild != NULL) {
        dfs_traverse(root, func, acc);
    }
...
int print_node(bstnode_t* node) {
    printf("%d\n", node->data);
    return 0;
}

int get_node_data(bstnode_t* node) {
    return node->data;
}
